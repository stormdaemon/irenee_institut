-- Paid library memberships and student book requests.
...
alter table if exists public.book_requests add column if not exists library_membership_id uuid references public.library_memberships(id) on delete set null;

create index if not exists library_memberships_user_status_expires_idx
  on public.library_memberships (user_id, status, expires_at desc);

create index if not exists book_requests_user_requested_at_idx
  on public.book_requests (user_id, requested_at desc);
